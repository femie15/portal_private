<x-ux::layouts.page :title="__('Parents')">
    <x-ux::actions search>
        <x-ux::button icon="plus" :title="__('Add New Parent')" click="$emit('showModal', 'users.saveparent')" label="Add New Parent"/>
        ...
        @foreach($users as $user)
            <x-ux::list-row>
                <x-ux::column margin="3">
                    ...
                    <x-ux::link :label="$user->name" click="$emit('showModal', 'users.readparent', {{ $user->id }})"/>
                    <x-ux::item icon="home" iconColor="primary" :data="$user->address" size="small" color="muted"/>
                </x-ux::column>
                ...
                <x-ux::action-column>
                    <x-ux::action icon="eye" :title="__('Read')" click="$emit('showModal', 'users.readparent', {{ $user->id }})"/>
                    <x-ux::action icon="pencil-alt" :title="__('Update')" click="$emit('showModal', 'users.saveparent', {{ $user->id }})"/>
