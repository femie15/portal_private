<x-ux::layouts.page>  
    {{-- <x-ux::layouts.page :title="__('Midterm Bulk Result')">   --}}
    <style>
    .table-resp {
    width: 65%;
    overflow-x: auto;
...
@else
    @php
        ...
        $getStudent='\App\Components\Results\Midtermbulk'::getStudent();        
        $promotion_status='';
        $ft=$sc=$tt=$sn=$av=0;
...
<script type="text/javascript">
    ...
    $ft=$sc=$tt=$sn=$av=0;
            ...
            $aa= $getStud->uname.' | Midterm Report';
            $sectn= $getStud->sname;
            $classn= $getStud->cname;
            $roll= $getStud->roll;
    ...
    $position='\App\Components\Results\Midtermbulk'::getposition($exp[0],$expt[0],$stuid);
   
    if ($expt[0]=='1st') {
            $ltposition=$position;
    }elseif ($expt[0]=='2nd') {
            $ltposition='\App\Components\Results\Midtermbulk'::getposition($exp[0],'1st',$stuid);
    }elseif ($expt[0]=='3rd'){
            $ltposition='\App\Components\Results\Midtermbulk'::getposition($exp[0],'2nd',$stuid);
    }else {
        $ltposition='';
    }
        $results='\App\Components\Results\Midtermbulk'::rst($stuid);        
    @endphp
...
$firstterm='\App\Components\Results\Midtermbulk'::termly($result->student_id,'1st',$result->session,$result->subject_id);
$secondterm='\App\Components\Results\Midtermbulk'::termly($result->student_id,'2nd',$result->session,$result->subject_id);
$thirdterm='\App\Components\Results\Midtermbulk'::termly($result->student_id,'3rd',$result->session,$result->subject_id);
...
@endphp
        ...
        @if ($classn =='JSS1' || $classn =='JSS2' || $classn =='JSS3')
            <td><center>{{ '\App\Components\Results\Midtermbulk'::gradej($sum)['grade'] }} </center></td>
           <td><center>{{ '\App\Components\Results\Midtermbulk'::gradej($sum)['remark'] }}</center></td>                
        @else
            <td><center>{{ '\App\Components\Results\Midtermbulk'::grade($sum)['grade'] }} </center></td>
            <td><center>{{ '\App\Components\Results\Midtermbulk'::grade($sum)['remark'] }}</center></td>                
        @endif
